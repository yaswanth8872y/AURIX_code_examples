...
#endif
	(unsigned int *)TRICORE_6_HANDSHAKE_MAILBOX
};
...
int IfxTcIpc_testRun(int *ptr[])
{
	return 0;
}
/* Entry point function table - These will be called by RPC handler */
int (*IfxTcIpc_services[IFXTCIPC_MAX_NUM_SERVICES])(int *ptr[]) =
{
		  /* Add the services */
		IfxTcIpc_testRun
...
void IfxTcIpc_processRemoteCall(IfxMailbox *mbx)
{
	int status = 1;
    if((mbx->command == IfxMailbox_Command_new) && (mbx->status == IfxMailbox_Status_requestDone))
    {
#if (IFX_VP_DEBUG_PRINT == 1U)
      printf("\n log : A new command is received from mailbox %x\n",(unsigned int)mbx);
      fflush(stdout);
#endif
      /* Let the host know that the job has been taken up */
      mbx->status = IfxMailbox_Status_requestInProgress;

      /* Invoke the corresponding C wrapper */
      status = (*IfxTcIpc_services[mbx->id])(mbx->args);

      /* Let the host know that the job is completed */
      if(status == 0)
      {
        mbx->status = IfxMailbox_Status_successful;
      }
      else
      {
        mbx->status = IfxMailbox_Status_notsuccessful;
      }
    }
}
